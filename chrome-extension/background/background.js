...
async function handleMessage(message, sender, sendResponse) {
    ...
    switch (message.type) {
        ...
        case 'UPDATE_SETTINGS':
            await storage.updateSettings(message.data);
            sendResponse({ success: true });
            break;
        case 'START_SESSION':
            const sessionId = await startSession(message.data);
...
async function toggleCopilot(tabId) {
    if (!tabId)
        return;
    try {
...
async function broadcastToContent(type, data) {
    try {
        const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
        if (tab.id) {
